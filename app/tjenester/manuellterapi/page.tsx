...
export default function ManuellerapiPage() {
  ...
  return (
    <>
            ...
            <div className="lg:col-span-2">
              ...
              <div className="mb-8 flex flex-wrap gap-2">
                {service.audiences.map((a) => (
                  <span key={a} className="rounded-full bg-moss-50 px-3 py-1 text-xs font-medium text-moss-700">{a}</span>
                ))}
              </div>
        ...
        <section className="bg-bone py-16 md:py-20">
          <Container>
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {relatedTherapists.map((t) => (
                <AnimatedSection key={t.slug}><TherapistCard therapist={t} /></AnimatedSection>
              ))}
            </div>
          </Container>
        </section>
      ...
      <CtaBanner heading={`Klar for å bestille time til ${service.title.toLowerCase()}?`} body="Ring oss direkte eller bruk kontaktskjemaet — vi finner en tid som passer." />
    </>
  );
}
