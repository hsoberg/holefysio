...
export default function FysioterapiPage() {
  ...
  const relatedTherapists = therapists.filter((t) =>
    service.therapistSlugs.includes(t.slug)
  );
  ...
  return (
    <>
        ...
        <Container>
          ...
          <div className="grid gap-12 lg:grid-cols-3">
            <div className="lg:col-span-2">
              ...
              <p className="speakable-answer text-lg text-ink-muted leading-relaxed mb-8">
                {service.leadAnswer}
              </p>
              ...
              <div className="mb-8 flex flex-wrap gap-2">
                {service.audiences.map((a) => (
                  <span key={a} className="rounded-full bg-moss-50 px-3 py-1 text-xs font-medium text-moss-700">
                    {a}
                  </span>
                ))}
              </div>

              <div className="prose text-ink-muted mb-8">
                <p>{service.intro}</p>
              </div>
                ...
                <ul className="flex flex-col gap-3">
                  {service.benefits.map((b) => (
                    <li key={b} className="flex items-start gap-3 text-sm text-ink-muted">
                      <svg className="h-5 w-5 shrink-0 text-moss-600 mt-0.5" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24 rounded-2xl bg-bone p-6 shadow-soft">
                ...
                {relatedTherapists.map((t) => (
                  <div key={t.slug} className="mb-4 last:mb-0">
                    ...
                    <a
                      ...
                      className="inline-flex items-center gap-2 text-sm font-medium text-terra-500 link-underline"
                    >
                ...
                <div className="mt-6 border-t border-line pt-5">
                  <a
                    href="/kontakt"
                    className="flex h-11 w-full items-center justify-center rounded-full bg-moss-600 text-sm font-medium text-paper hover:bg-moss-700 transition-colors"
                  >
                    Kontaktskjema
                  </a>
                </div>
      ...
      <FaqSection items={service.faq} heading={`Vanlige spørsmål om ${service.title.toLowerCase()}`} />

      <CtaBanner
        heading={`Klar for å bestille time til ${service.title.toLowerCase()}?`}
        body="Ring oss direkte eller bruk kontaktskjemaet — vi finner en tid som passer."
      />
    </>
  );
}
